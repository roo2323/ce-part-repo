...
import type { VaultItem, VaultCategory, UpdateVaultRequest } from '@/types';
...
export default function EditVaultItemScreen() {
  ...
  const { currentItem, fetchVaultItem, updateVaultItem, isLoading } = useVaultStore();
  ...
  const loadVaultItem = async () => {
    ...
    try {
      ...
      setIncludeInAlert(item.includeInAlert);
    } catch (error) {
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.container}>
        <Stack.Screen options={{ title: '정보 상세' }} />
