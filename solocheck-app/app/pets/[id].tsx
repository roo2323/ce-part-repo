...
import type { Pet, PetSpecies, UpdatePetRequest } from '@/types';
...
export default function EditPetScreen() {
  ...
  const { updatePet, isLoading } = usePetStore();
  ...
  const loadPet = async () => {
    ...
    try {
      ...
      setVetInfo(data.vetInfo || '');
      setCaretakerContact(data.caretakerContact || '');
      setIncludeInAlert(data.includeInAlert);
    } catch (error) {
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.container}>
        <Stack.Screen options={{ title: '반려동물 정보' }} />
